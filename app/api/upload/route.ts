...
export async function POST(req: Request) {
  try {
    ...
    // Защита от "гонки" данных Supabase
    if (!insertData || !insertData.id) {
      console.warn("[Upload] Record created but ID not returned immediately.");
      return NextResponse.json({ 
        success: true, 
        message: "Image uploaded, but analysis skipped due to missing ID." 
      });
    }
    ...
    return NextResponse.json({ 
      success: true, 
      id: insertData.id,
      message: "Image uploaded. AI analysis started in background." 
    });
