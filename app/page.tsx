...
export default function Home() {
  ...
  function handleTagClick(tag: string) { 
    if(oracleMode) { 
      setOracleMode(false); 
      setOracleData(prev => ({ ...prev, isIdle: true, trackName: "Awaiting sonic input..." })); 
    }
    setSearch(tag); 
    setSearchQuery(tag); 
    saveUserTag(tag); 
  ...
  async function handleAIGenerate() {
    ...
    try {
      const aiRes = await fetch("/api/ai", { 
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
        body: JSON.stringify({ action: "enhance_prompt", payload: finalQuery }) 
      });
      ...
      if (aiRes.ok) { 
        ...
        if (!/[а-яА-ЯёЁ]/.test(finalQuery) && !finalQuery.toLowerCase().includes('aesthetic')) {
          finalQuery += " aesthetic"; 
        }
      }
    } catch (e) { 
      showToast("AI network error."); 
        ...
        if(oracleMode) { 
          setOracleMode(false); 
          setOracleData(prev => ({ ...prev, isIdle: true })); 
        }
        setSearch(finalQuery); 
        setSearchQuery(finalQuery); 
        saveUserTag(finalQuery); 
        setAiPrompt(""); 
        setIsMobileSearchOpen(false); 
        setIsSearchFocused(false);
        setPage(1); 
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes volvelle-spin { 
          100% { transform: translate(-50%, -50%) rotate(360deg); } 
        }
        @keyframes heartbeat { 
          0%, 100% { transform: scale(1); opacity: 0.5; box-shadow: none; } 
          10%, 30% { transform: scale(1.2); opacity: 1; box-shadow: 0 0 20px rgba(249, 115, 22, 0.8); } 
        ...
        .header { 
          ...
          background: rgba(2, 1, 4, 0.8); backdrop-filter: blur(20px); 
          border-bottom: 1px solid rgba(255,255,255,0.05); 
          padding: 12px 24px; display: flex; align-items: center; justify-content: space-between; 
        }
        
        .logo { 
          font-family: 'Syncopate', sans-serif; font-size: 20px; font-weight: 700; color: #fff; 
          letter-spacing: 0.3em; cursor: pointer; user-select: none; 
          text-shadow: 0 0 10px rgba(255,255,255,0.2); transition: all 0.3s; 
        }
        .logo:hover { 
          text-shadow: -3px 0 10px rgba(255,0,85,0.5), 0 0 10px rgba(0,255,0,0.5), 3px 0 10px rgba(0,255,255,0.5); 
        }
        ...
        .search-wrap { 
          ...
          background: rgba(255,255,255,0.02); border-radius: 9999px; 
          border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 0 20px rgba(0,0,0,0.8); 
          transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); 
        } 
        .search-wrap.prism-focus:focus-within { 
          border-color: rgba(255,255,255,0.3); background: rgba(255,255,255,0.05); 
          box-shadow: -10px 0 30px rgba(255,0,85,0.2), 0 0 30px rgba(0,255,0,0.1), 10px 0 30px rgba(0,255,255,0.2); 
        }
        .search-wrap.oracle-active { 
          border-color: #f97316; box-shadow: 0 0 30px rgba(249, 115, 22, 0.3); 
        }
        
        .oracle-toggle-inline { 
          display: flex; align-items: center; justify-content: center; padding: 0 12px 0 16px; 
          border-right: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.5); 
          background: transparent; cursor: pointer; transition: color 0.2s; 
          height: 24px; margin: 8px 0; flex-shrink: 0; 
        }
        .oracle-toggle-inline:hover, .search-wrap.oracle-active .oracle-toggle-inline { 
          color: #fff; text-shadow: 0 0 10px #fff; 
        }
        
        .search-input { 
          width: 100%; padding: 12px 16px; background: transparent; border: none; 
          color: #fff; font-size: 14px; outline: none; transition: all 0.3s ease; font-family: 'Inter', sans-serif; 
        } 
        ...
        .search-btn { 
          width: 40px; height: 100%; padding: 0; background: transparent; border: none; 
        ...
        .search-dropdown { 
          ...
          background: rgba(2,1,4,0.95); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; 
          box-shadow: 0 20px 50px rgba(0,0,0,0.9), 0 0 30px rgba(168,85,247,0.1); 
          backdrop-filter: blur(20px); z-index: 300; padding: 8px 0; 
          animation: slideUp 0.3s cubic-bezier(0.16,1,0.3,1); overflow: hidden; 
        }
        .search-dropdown-header { 
          display: flex; justify-content: space-between; align-items: center; 
          padding: 12px 16px 8px; font-size: 10px; font-weight: 700; color: #666; 
          text-transform: uppercase; letter-spacing: 2px; font-family: 'Syncopate', sans-serif; 
        }
        ...
        .search-dropdown-clear-all:hover { color: #ef4444; }
        
        .search-dropdown-item { 
          display: flex; align-items: center; padding: 12px 16px; cursor: pointer; 
          transition: all 0.2s; gap: 12px; color: #ddd; font-size: 14px; 
        } 
        .search-dropdown-item:hover { background: rgba(255,255,255,0.05); color: #fff; padding-left: 20px; }
        .search-dropdown-icon { font-size: 14px; color: #666; } 
        ...
        .search-dropdown-remove { 
          background: none; border: none; color: #666; cursor: pointer; width: 26px; height: 26px; 
          border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.2s; 
        } 
        .search-dropdown-remove:hover { color: #ef4444; background: rgba(239,68,68,0.1); }
        
        .oracle-stage-wrapper { 
          width: 100%; padding: 0 16px; height: 0; opacity: 0; overflow: hidden; 
          transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); position: relative; z-index: 10; 
        }
        ...
        .oracle-meta-title { 
          font-family: 'Syncopate', sans-serif; font-size: 10px; letter-spacing: 6px; 
          color: #555; text-transform: uppercase; display: flex; align-items: center; gap: 12px; transition: color 1s ease; 
        }
        ...
        .oracle-track-name { 
          font-family: 'Syncopate', sans-serif; font-size: 16px; color: #555; 
          margin-top: 12px; letter-spacing: 2px; transition: all 1s ease; text-align: center; 
          max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; text-transform: uppercase; 
        }
        .oracle-track-name.active { color: #fff; font-weight: 700; text-shadow: 0 0 20px rgba(255,255,255,0.4); }

        @media (max-width: 768px) { 
          .search-form { flex: none; margin: 0; } 
          .search-container { flex: none; } 
          .search-wrap { width: 40px; height: 40px; flex: none; border-radius: 50%; background: transparent; border: none; cursor: pointer; box-shadow: none; } 
          .search-input { display: none; } 
          .search-btn { width: 40px; height: 40px; border-radius: 50%; pointer-events: none; margin: 0; } 
          .oracle-toggle-inline { display: none; } 
          ...
          .search-form.mobile-active .search-wrap { width: 100%; height: 48px; border-radius: 9999px; background: rgba(2,1,4,0.95); border: 1px solid rgba(255,255,255,0.2); flex: 1; display: flex; cursor: text; box-shadow: 0 20px 40px rgba(0,0,0,0.9); } 
          .search-form.mobile-active .search-input { display: block; flex: 1; } 
          .search-form.mobile-active .search-btn { width: 44px; pointer-events: auto; } 
          .search-form.mobile-active .oracle-toggle-inline { display: flex; } 
        ...
        .hbtn { 
          ...
          color: #888; flex-shrink: 0; transition: all 0.3s ease; position: relative; 
        } 
        ...
        .avatar { width: 34px; height: 34px; border-radius: 50%; cursor: pointer; border: 1px solid rgba(255,255,255,0.2); flex-shrink: 0; object-fit: cover; transition: all 0.3s; } 
        .avatar:hover { border-color: #d4b896; box-shadow: 0 0 15px rgba(212,184,150,0.4); }
        
        .avatar-placeholder { width: 34px; height: 34px; border-radius: 50%; cursor: pointer; border: 1px solid rgba(255,255,255,0.2); flex-shrink: 0; background: #111; display: flex; align-items: center; justify-content: center; color: #888; font-size: 13px; font-weight: 700; font-family: 'Syncopate', sans-serif; transition: all 0.3s; } 
        .avatar-placeholder:hover { border-color: #d4b896; color: #d4b896; }
        
        .sign-btn { background: transparent; color: #fff; border: 1px solid rgba(255,255,255,0.3); border-radius: 20px; padding: 8px 20px; cursor: pointer; font-size: 11px; font-family: 'Syncopate', sans-serif; font-weight: 700; flex-shrink: 0; text-decoration: none; display: flex; align-items: center; transition: all 0.3s; letter-spacing: 1px; } 
        .sign-btn:hover { border-color: #fff; background: #fff; color: #000; box-shadow: 0 0 20px rgba(255,255,255,0.2); }
        
        .tag-pill { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); color: #aaa; padding: 8px 18px; border-radius: 24px; font-size: 11px; font-weight: 600; font-family: 'Syncopate', sans-serif; letter-spacing: 1px; white-space: nowrap; cursor: pointer; transition: all 0.3s cubic-bezier(0.16,1,0.3,1); } 
        .tag-pill:hover { background: rgba(255,255,255,0.1); color: #fff; transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.5); } 
        .tag-pill.active { background: rgba(168,85,247,0.15); color: #a855f7; border-color: #a855f7; box-shadow: 0 0 20px rgba(168,85,247,0.2); }
        
        .burger-overlay { position: fixed; inset: 0; z-index: 150; background: rgba(2,1,4,0.9); animation: fadeIn 0.3s ease; backdrop-filter: blur(10px); } 
        .burger-panel { position: fixed; top: 0; left: 0; bottom: 0; width: min(340px, 85vw); z-index: 151; background: #050308; border-right: 1px solid rgba(255,255,255,0.05); display: flex; flex-direction: column; animation: slideRight 0.4s cubic-bezier(0.16,1,0.3,1); overflow-y: auto; box-shadow: 20px 0 50px rgba(0,0,0,0.8); }
        
        @keyframes slideRight { from { transform: translateX(-100%); } to { transform: translateX(0); } } 
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } 
        @keyframes slideUp { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        
        .burger-header { padding: 24px 24px 20px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: space-between; } 
        .burger-logo { font-family: 'Syncopate', sans-serif; font-size: 16px; font-weight: 700; letter-spacing: 4px; color: #fff; } 
        .burger-close { background: none; border: none; font-size: 18px; cursor: pointer; color: #666; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.3s; } 
        .burger-close:hover { color: #fff; background: rgba(255,255,255,0.1); transform: rotate(90deg); } 
        .burger-action { display: flex; align-items: center; gap: 16px; padding: 16px 24px; cursor: pointer; border: none; background: none; width: 100%; text-align: left; color: #aaa; font-size: 13px; font-weight: 600; font-family: 'Syncopate', sans-serif; letter-spacing: 1px; transition: all 0.2s; text-transform: uppercase; } 
        .burger-action:hover { background: rgba(255,255,255,0.05); color: #fff; padding-left: 32px; } 
        .burger-action-icon { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); transition: all 0.3s; } 
        .burger-action:hover .burger-action-icon { border-color: #a855f7; color: #a855f7; box-shadow: 0 0 15px rgba(168,85,247,0.3); }
        
        .grid-wrap { padding: 24px; position: relative; z-index: 10; max-width: 1600px; margin: 0 auto; } 
        ...
        @media (min-width: 1440px) { .masonry { columns: 6; } }
        
        .boards-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; position: relative; z-index: 10; max-width: 1400px; margin: 0 auto; padding: 0 24px; } 
        @media (min-width: 640px) { .boards-grid { grid-template-columns: repeat(3, 1fr); } } 
        @media (min-width: 1024px) { .boards-grid { grid-template-columns: repeat(4, 1fr); } } 
        
        .board-card { border-radius: 16px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); background: rgba(10,10,12,0.8); backdrop-filter: blur(20px); transition: all 0.4s cubic-bezier(0.16,1,0.3,1); box-shadow: 0 10px 30px rgba(0,0,0,0.5); } 
        .board-card:hover { transform: translateY(-5px); border-color: rgba(255,255,255,0.15); box-shadow: -10px 0 30px rgba(255,0,85,0.1), 0 0 30px rgba(0,255,0,0.05), 10px 0 30px rgba(0,255,255,0.1); } 
        
        .board-cover { height: 100px; background: linear-gradient(135deg, #110d14, #050308); display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; } 
        .board-cover::after { content: ''; position: absolute; inset: 0; background: repeating-linear-gradient(-45deg, transparent, transparent 5px, rgba(255,255,255,0.02) 6px, rgba(255,255,255,0.02) 7px); }
        
        .board-info { padding: 16px; } 
        .board-actions { display: flex; gap: 8px; margin-top: 16px; } 
        
        .board-edit-btn, .board-del-btn { flex: 1; padding: 8px; border-radius: 8px; font-size: 10px; cursor: pointer; background: transparent; font-weight: 700; text-transform: uppercase; font-family: 'Syncopate', sans-serif; letter-spacing: 1px; transition: all 0.2s; } 
        .board-edit-btn { border: 1px solid rgba(255,255,255,0.1); color: #888; } 
        .board-edit-btn:hover { border-color: #fff; color: #fff; background: rgba(255,255,255,0.05); } 
        .board-del-btn { border: 1px solid rgba(239,68,68,0.3); color: #ef4444; } 
        .board-del-btn:hover { background: #ef4444; color: #000; box-shadow: 0 0 15px rgba(239,68,68,0.4); }
        ...
        .primary-btn:hover { background: #ccc; transform: translateY(-2px); box-shadow: 0 15px 30px rgba(255,255,255,0.2); } 
        .primary-btn:active { transform: translateY(0); } 
        .primary-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; box-shadow: none; }
        
        .ghost-btn { border-radius: 8px; padding: 14px 28px; cursor: pointer; font-weight: 700; font-size: 12px; transition: all 0.3s; background: transparent; text-transform: uppercase; letter-spacing: 2px; color: #888; border: 1px solid rgba(255,255,255,0.1); font-family: 'Syncopate', sans-serif; } 
        .ghost-btn:hover { background: rgba(255,255,255,0.05); color: #fff; border-color: rgba(255,255,255,0.3); } 
        
        .field { width: 100%; padding: 14px 20px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.5); color: #fff; font-size: 14px; outline: none; transition: all 0.3s ease; font-family: 'Inter', sans-serif; box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); } 
        .field:focus { border-color: #a855f7; background: rgba(168,85,247,0.05); box-shadow: 0 0 15px rgba(168,85,247,0.2), inset 0 2px 4px rgba(0,0,0,0.5); } 
        .field::placeholder { color: rgba(255,255,255,0.2); }
        
        .spinner { width: 32px; height: 32px; border: 2px solid rgba(255,255,255,0.1); border-top-color: #a855f7; border-radius: 50%; animation: spin 0.8s cubic-bezier(0.6, 0.2, 0.4, 0.8) infinite; margin: 0 auto; box-shadow: 0 0 15px rgba(168,85,247,0.5); } 
        @keyframes spin { to { transform: rotate(360deg); } }
        
        .empty { text-align: center; padding: 120px 20px; color: #555; font-size: 18px; position: relative; z-index: 10; font-family: 'Syncopate', sans-serif; text-transform: uppercase; letter-spacing: 2px; } 
        
        .modal-close { background: none; border: none; color: #666; cursor: pointer; font-size: 24px; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.3s; } 
      ...
      <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <header className="header">
                  ...
                  {userTags.length > 0 && (
                    <>
                      <div className="search-dropdown-header">
                        <span>Recent Frequencies</span>
                        <button type="button" onClick={clearAllTags} className="search-dropdown-clear-all">Purge</button>
                      </div>
                      {userTags.map(tag => (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <button className="hbtn" title="AI Vibe Assistant" onClick={() => setShowAIModal(true)}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l1.912 5.813a2 2 0 001.275 1.275L21 12l-5.813 1.912a2 2 0 00-1.275 1.275L12 21l-1.912-5.813a2 2 0 00-1.275-1.275L3 12l5.813-1.912a2 2 0 001.275-1.275L12 3z"/></svg>
            </button>
            ...
            <button className={`hbtn ${showSaved ? "active" : ""}`} onClick={() => { setShowSaved(!showSaved); setShowBoards(false); }}>
              <svg width="19" height="19" viewBox="0 0 24 24" fill={showSaved ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
              {pins.length > 0 && <span className="badge">{pins.length}</span>}
            </button>
            {user ? (
              <a href="/profile" style={{ marginLeft: 8 }}>
                {userAvatar ? <img src={userAvatar} className="avatar" alt="avatar" /> : <div className="avatar-placeholder">{(userName[0] || "U").toUpperCase()}</div>}
              </a>
            ) : (
              <a href="/auth" className="sign-btn" style={{ marginLeft: 8 }}>Initialize</a>
            )}
          </div>
        </header>
        ...
        {(showSaved || showBoards) && (
          <div style={{ padding: "16px 32px", fontSize: 11, color: "#888", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 16, position: "relative", zIndex: 10, fontFamily: 'Syncopate', letterSpacing: '2px', textTransform: 'uppercase' }}>
            {showSaved && <span>Saved Collection (<span style={{ color: "#fff" }}>{pins.length}</span>)</span>}
            {showBoards && <span>My Archives (<span style={{ color: "#fff" }}>{boards.length}</span>)</span>}
          </div>
        )}

        {showBoards && (
          <div className="boards-grid" style={{ paddingTop: 32 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, gridColumn: "1 / -1" }}>
              <h2 style={{ fontSize: 20, fontWeight: 700, color: "#fff", fontFamily: "Syncopate, sans-serif", letterSpacing: '4px' }}>MY ARCHIVES</h2>
              <button className="primary-btn" style={{ width: "auto", padding: "10px 20px", fontSize: 10 }} onClick={() => setShowNewBoard(true)}>+ ESTABLISH NEW</button>
            </div>
            {boards.length === 0 ? <div className="empty" style={{ gridColumn: "1 / -1" }}>No archives yet. Create your first collection.</div> : (
              boards.map(board => (
                <div key={board.id} className="board-card">
                  <div className="board-cover">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                  </div>
                  <div className="board-info">
                    <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: 'Syncopate', letterSpacing: '1px', textTransform: 'uppercase' }}>{board.name}</div>
                    {board.description && <div style={{ fontSize: 12, color: "#888", marginTop: 8, lineHeight: 1.5 }}>{board.description}</div>}
                    <div style={{ fontSize: 10, color: "#a855f7", marginTop: 12, fontWeight: 700, letterSpacing: '1px' }}>{pins.filter(p => p.board_id === board.id).length} ARTIFACTS</div>
                    <div className="board-actions">
                      <button className="board-edit-btn" onClick={() => setEditBoard(board)}>Edit</button>
                      <button className="board-del-btn" onClick={() => deleteBoard(board.id)}>Purge</button>
                    </div>
                  </div>
                </div>
              ))
        ...
        {!showBoards && (
          <>
            <div className="grid-wrap">
              <div className="masonry">
                {displayPhotos.map((photo, i) => (
                  <div key={`${photo.id}-${i}`} className="mb-4 rounded-xl overflow-hidden hover:scale-[1.02] transition-transform duration-300" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
                    <PinCard 
                      photo={photo} 
            ...
            {displayPhotos.length === 0 && !loading && <div className="empty">{showSaved ? "Archive is empty." : "Frequency not found."}</div>}
            {!showSaved && <div ref={bottomRef} style={{ padding: "60px", textAlign: "center", position: "relative", zIndex: 10 }}>{loading && <div className="spinner" />}</div>}
          </>
        )}
        ...
        {showMenu && (
          <>
            ...
            <div className="burger-panel">
              ...
              {user && (
                <div style={{ padding: "32px 24px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 16 }}>
                  {userAvatar ? <img src={userAvatar} className="avatar" style={{ width: 56, height: 56 }} alt="" /> : <div className="avatar-placeholder" style={{ width: 56, height: 56, fontSize: 20 }}>{(userName[0] || "U").toUpperCase()}</div>}
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 16, color: "#fff", fontFamily: 'Syncopate', letterSpacing: '1px', textTransform: 'uppercase' }}>{userName}</div>
                    <a href="/profile" style={{ color: "#a855f7", fontSize: 11, textDecoration: "none", marginTop: 6, display: "inline-block", fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>Configure Identity</a>
                  </div>
                </div>
              )}
              <div style={{ padding: "0 24px", fontSize: 10, color: "#888", textTransform: "uppercase", letterSpacing: 3, marginTop: 32, marginBottom: 20, fontFamily: 'Syncopate', fontWeight: "bold" }}>Global Resonance</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, padding: "0 24px 32px" }}>
              ...
              <div style={{ padding: "16px 0" }}>
                <button className="burger-action" onClick={() => { setShowBoards(true); setShowMenu(false); }}>
                  <span className="burger-action-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></span><span>My Archives</span>
                </button>
                <button className="burger-action" onClick={() => { setShowSaved(true); setShowMenu(false); }}>
                  <span className="burger-action-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></span><span>Saved Artifacts</span>
                </button>
              </div>
            </div>
          </>
          ...
          <div className="modal-backdrop" onClick={() => setShowAIModal(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: "rgba(2,1,4,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 40, maxWidth: 500, width: "100%", display: "flex", flexDirection: "column", gap: 24, animation: "slideUp 0.4s cubic-bezier(0.16,1,0.3,1)", boxShadow: "0 30px 60px rgba(0,0,0,0.9), 0 0 30px rgba(168,85,247,0.1)", backdropFilter: "blur(20px)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: 16, fontWeight: 700, color: "#a855f7", fontFamily: "Syncopate, sans-serif", letterSpacing: 3 }}>AI VIBE ASSISTANT</h2>
                <button className="modal-close" onClick={() => setShowAIModal(false)}>✕</button>
              </div>
              <textarea className="field" placeholder="Describe a specific mood, esoteric concept, or aesthetic..." value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} style={{ height: 120, resize: "none", fontSize: 16 }} />
              <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
                <button className="ghost-btn" style={{ flex: 1 }} onClick={() => setShowAIModal(false)}>ABORT</button>
                <button className="primary-btn" style={{ flex: 1.5, opacity: !aiPrompt.trim() ? 0.4 : 1 }} onClick={handleAIGenerate}>SYNTHESIZE</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showSaveToBoard && (
          <div className="modal-backdrop" onClick={() => setShowSaveToBoard(null)}>
            <div onClick={e => e.stopPropagation()} style={{ background: "rgba(2,1,4,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 40, maxWidth: 460, width: "100%", display: "flex", flexDirection: "column", gap: 20, boxShadow: "0 30px 60px rgba(0,0,0,0.9)", backdropFilter: "blur(20px)" }}>
              <h2 style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "Syncopate, sans-serif", letterSpacing: 3, textAlign: 'center', marginBottom: 10 }}>STORE ARTIFACT</h2>
              <button className="primary-btn" onClick={() => savePin(showSaveToBoard)}>SAVE DIRECTLY TO PROFILE</button>
              <div style={{ textAlign: 'center', color: '#666', fontSize: 10, fontFamily: 'Syncopate', letterSpacing: 2 }}>— OR —</div>
              <button className="ghost-btn" onClick={() => { setShowNewBoard(true); setShowSaveToBoard(null); }}>+ ESTABLISH NEW ARCHIVE</button>
            </div>
          </div>
        )}
        
        {isAILoading && (
          <div className="ai-global-loader">
