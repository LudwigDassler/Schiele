...
export default function Home() {
  ...
  function sharePhoto(photo: Photo) { const url = photo.link || window.location.href; if (navigator.share) navigator.share({ title: photo.title || "Gelbet Vibe", url }); else { navigator.clipboard.writeText(url); showToast("Link copied to clipboard!"); } setShowShare(null); }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body::after { content: ""; position: fixed; inset: 0; z-index: 9999; pointer-events: none; opacity: 0.04; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E"); }
        .header { position: sticky; top: 0; z-index: 100; background: rgba(13,10,6,0.95); backdrop-filter: blur(12px); border-bottom: 1px solid #2a1f0e; padding: 10px 16px; display: flex; align-items: center; gap: 10px; }
        ...
        .search-wrap { flex: 1; display: flex; background: #1a1208; border-radius: 24px; overflow: hidden; border: 1px solid #2a1f0e; transition: all 0.2s; min-width: 0; } .search-wrap:focus-within { border-color: #c0521a; box-shadow: 0 0 0 2px rgba(192,82,26,0.2); }
        ...
        .search-btn { width: 40px; height: 100%; padding: 0; background: transparent; border: none; color: #6a4a2a; cursor: pointer; transition: color 0.2s; display: flex; align-items: center; justify-content: center; flex-shrink: 0; } .search-btn:hover { color: #c0521a; }
        .search-dropdown { position: absolute; top: calc(100% + 8px); right: 0; width: 100%; max-width: 440px; background: rgba(13,10,6,0.98); border: 1px solid #2a1f0e; border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.8); backdrop-filter: blur(12px); z-index: 300; padding: 8px 0; animation: slideUp 0.2s ease; overflow: hidden; }
        ...
        @media (max-width: 640px) { .search-form { flex: none; } .search-container { flex: none; } .search-wrap { width: 38px; height: 38px; flex: none; border-radius: 50%; background: transparent; border: none; cursor: pointer; } .search-input { display: none; } .search-btn { width: 38px; height: 38px; border-radius: 50%; pointer-events: none; } .search-form.mobile-active { position: absolute; left: 16px; right: 16px; top: 10px; z-index: 200; flex: 1; } .search-form.mobile-active .search-container { flex: 1; } .search-form.mobile-active .search-wrap { width: 100%; height: 42px; border-radius: 24px; background: #0d0a06; border: 1px solid #c0521a; flex: 1; display: flex; cursor: text; box-shadow: 0 10px 30px rgba(0,0,0,0.9); } .search-form.mobile-active .search-input { display: block; flex: 1; } .search-form.mobile-active .search-btn { border-radius: 0; width: 40px; pointer-events: auto; } .search-dropdown { max-width: 100%; } }
        .hbtn { background: transparent; border: none; width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #6a4a2a; flex-shrink: 0; transition: all 0.2s; position: relative; } .hbtn:hover { background: #1a1208; color: #c0521a; } .hbtn.active { background: #c0521a; color: #0d0a06; }
        ...
        .avatar { width: 32px; height: 32px; border-radius: 50%; cursor: pointer; border: 2px solid #2a1f0e; flex-shrink: 0; object-fit: cover; } .avatar-placeholder { width: 32px; height: 32px; border-radius: 50%; cursor: pointer; border: 2px solid #2a1f0e; flex-shrink: 0; background: #c0521a; display: flex; align-items: center; justify-content: center; color: #0d0a06; font-size: 13px; font-weight: 700; }
        ...
        .tag-pill { background: #1a1208; border: 1px solid #2a1f0e; color: #d4b896; padding: 6px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; white-space: nowrap; cursor: pointer; transition: all 0.2s; } .tag-pill:hover { background: #2a1f0e; color: #c0521a; border-color: #4a3520; } .tag-pill.active { background: #c0521a; color: #0d0a06; border-color: #c0521a; }
        .burger-overlay { position: fixed; inset: 0; z-index: 150; background: rgba(0,0,0,0.7); animation: fadeIn 0.2s ease; } .burger-panel { position: fixed; top: 0; left: 0; bottom: 0; width: min(300px, 85vw); z-index: 151; background: #0d0a06; border-right: 1px solid #2a1f0e; display: flex; flex-direction: column; animation: slideRight 0.25s ease; overflow-y: auto; }
        ...
        .burger-header { padding: 20px 20px 16px; border-bottom: 1px solid #2a1f0e; display: flex; align-items: center; justify-content: space-between; } .burger-logo { font-family: 'Cinzel', serif; font-size: 14px; font-weight: 700; letter-spacing: 4px; color: #c0521a; } .burger-close { background: none; border: none; font-size: 18px; cursor: pointer; color: #4a3520; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; } .burger-close:hover { color: #c0521a; background: #1a1208; } .burger-action { display: flex; align-items: center; gap: 12px; padding: 14px 16px; cursor: pointer; border: none; background: none; width: 100%; text-align: left; color: #8a6a4a; font-size: 13px; transition: all 0.15s; } .burger-action:hover { background: #1a1208; color: #d4b896; } .burger-action-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: #1a1208; }
        .grid-wrap { padding: 10px; } .masonry { columns: 2; gap: 8px; } @media (min-width: 480px) { .masonry { columns: 3; } } @media (min-width: 768px) { .masonry { columns: 4; } } @media (min-width: 1024px) { .masonry { columns: 5; } } @media (min-width: 1280px) { .masonry { columns: 6; } }
        .boards-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; } @media (min-width: 480px) { .boards-grid { grid-template-columns: repeat(3, 1fr); } } .board-card { border-radius: 12px; overflow: hidden; border: 1px solid #2a1f0e; background: #1a1208; } .board-cover { height: 80px; background: linear-gradient(135deg, #2a1f0e, #1a1208); display: flex; align-items: center; justify-content: center; } .board-info { padding: 12px; } .board-actions { display: flex; gap: 8px; margin-top: 10px; } .board-edit-btn, .board-del-btn { flex: 1; padding: 6px; border-radius: 6px; font-size: 11px; cursor: pointer; background: transparent; } .board-edit-btn { border: 1px solid #2a1f0e; color: #8a6a4a; } .board-edit-btn:hover { border-color: #c0521a; color: #c0521a; } .board-del-btn { border: 1px solid #3a1a1a; color: #e53e3e; } .board-del-btn:hover { background: rgba(229,62,62,0.1); }
        ...
        .modal-img-wrap { flex: 1.5; background: #1a1208; display: flex; align-items: center; justify-content: center; padding: 20px; } .modal-img { width: 100%; max-height: 70vh; object-fit: contain; display: block; border-radius: 8px; }
        .modal-info { flex: 1; padding: 32px; display: flex; flex-direction: column; background: #0d0a06; } .modal-bottom { padding: 32px; border-top: 1px solid #1a1208; background: #0d0a06; }
        ...
        .primary-btn { background: #c0521a; color: #0d0a06; border: none; border-radius: 24px; padding: 14px 24px; cursor: pointer; font-weight: 700; font-size: 14px; width: 100%; transition: background 0.2s; } .primary-btn:hover { background: #d4621a; } .primary-btn.pinned-state { background: #1a1208; color: #c0521a; border: 1px solid #c0521a; }
        .ghost-btn, .outline-btn, .danger-btn { border-radius: 20px; padding: 12px 24px; cursor: pointer; font-weight: 600; font-size: 14px; transition: all 0.2s; background: transparent; } .ghost-btn { color: #8a6a4a; border: 1px solid #2a1f0e; } .ghost-btn:hover { border-color: #4a3520; color: #d4b896; } .outline-btn { color: #c0521a; border: 1px solid #c0521a; width: 100%; } .outline-btn:hover { background: rgba(192,82,26,0.1); } .danger-btn { color: #e53e3e; border: 1px solid #e53e3e; width: 100%; } .danger-btn:hover { background: rgba(229,62,62,0.1); }
        .field { width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #2a1f0e; background: #1a1208; color: #d4b896; font-size: 14px; outline: none; transition: border-color 0.2s; } .field:focus { border-color: #c0521a; }
        .spinner { width: 28px; height: 28px; border: 2px solid #2a1f0e; border-top-color: #c0521a; border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto; } @keyframes spin { to { transform: rotate(360deg); } }
        .ai-pulse { width: 40px; height: 40px; border-radius: 50%; background: radial-gradient(circle, #c0521a 0%, transparent 70%); animation: pulseGlow 1.5s infinite alternate; margin: 0 auto 12px; } @keyframes pulseGlow { 0% { transform: scale(0.8); opacity: 0.5; box-shadow: 0 0 10px #c0521a; } 100% { transform: scale(1.5); opacity: 1; box-shadow: 0 0 30px #c0521a, 0 0 60px #d4b896; } } .ai-text { font-family: 'Cinzel', serif; color: #d4b896; font-size: 11px; letter-spacing: 4px; text-transform: uppercase; animation: flashText 1.5s infinite; } @keyframes flashText { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }
        .empty { text-align: center; padding: 100px 20px; color: #4a3520; font-size: 16px; font-family: 'Crimson Text', serif; font-style: italic; } .modal-close { background: none; border: none; color: #4a3520; cursor: pointer; font-size: 20px; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; } .modal-close:hover { background: #1a1208; color: #c0521a; }
      ...
      <main style={{ minHeight: "100vh" }}>
        <header className="header">
          ...
          <form className={`search-form ${isMobileSearchOpen ? 'mobile-active' : ''}`} onSubmit={handleSearch}>
            <div className="search-container" ref={searchContainerRef}>
              <div className="search-wrap" onClick={() => { if (!isMobileSearchOpen) { setIsMobileSearchOpen(true); setTimeout(() => searchInputRef.current?.focus(), 50); } }}>
                <input ref={searchInputRef} className="search-input" placeholder="Search visual aesthetics..." value={search} onChange={e => setSearch(e.target.value)} onFocus={() => setIsSearchFocused(true)} />
          ...
          <button className={`hbtn ${showSaved ? "active" : ""}`} onClick={() => { setShowSaved(!showSaved); setShowBoards(false); }}><svg width="17" height="17" viewBox="0 0 24 24" fill={showSaved ? "#0d0a06" : "none"} stroke={showSaved ? "#0d0a06" : "currentColor"} strokeWidth="2" strokeLinecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>{pins.length > 0 && <span className="badge">{pins.length}</span>}</button>
        ...
        {(showSaved || showBoards) && (<div style={{ padding: "12px 20px", fontSize: 14, color: "#8a6a4a", borderBottom: "1px solid #1a1208", display: "flex", alignItems: "center", gap: 12 }}>{showSaved && <span>Saved Collection (<span style={{ color: "#c0521a" }}>{pins.length}</span>)</span>}{showBoards && <span>My Boards (<span style={{ color: "#c0521a" }}>{boards.length}</span>)</span>}</div>)}

        {showBoards && (
          <div style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><h2 style={{ fontSize: 18, fontWeight: 700, color: "#d4b896", fontFamily: "Cinzel, serif", letterSpacing: 2 }}>MY BOARDS</h2><button className="primary-btn" style={{ width: "auto", padding: "10px 20px" }} onClick={() => setShowNewBoard(true)}>+ New Board</button></div>
            {boards.length === 0 ? <div className="empty">No boards yet. Create your first collection.</div> : (<div className="boards-grid">{boards.map(board => (<div key={board.id} className="board-card"><div className="board-cover"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#c0521a" strokeWidth="1.5"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></div><div className="board-info"><div style={{ fontSize: 14, fontWeight: 600, color: "#d4b896" }}>{board.name}</div>{board.description && <div style={{ fontSize: 11, color: "#4a3520", marginTop: 4, fontStyle: "italic" }}>{board.description}</div>}<div style={{ fontSize: 11, color: "#8a6a4a", marginTop: 8 }}>{pins.filter(p => p.board_id === board.id).length} pins</div><div className="board-actions"><button className="board-edit-btn" onClick={() => setEditBoard(board)}>Edit</button><button className="board-del-btn" onClick={() => deleteBoard(board.id)}>Delete</button></div></div></div>))}</div>)}
          </div>
        )}

        {!showBoards && (
          <><div className="grid-wrap"><div className="masonry">{displayPhotos.map((photo, i) => (<PinCard key={`${photo.id}-${i}`} photo={photo} nsfwAllowed={nsfwAllowed} isPinned={isPinned(photo)} showSaved={showSaved} onClick={() => { const isBlurred = photo.isNsfw && !nsfwAllowed; if (isBlurred) setShowAgeGate(true); else setSelected(photo); }} onSaveClick={(e: any) => { e.stopPropagation(); if (!isPinned(photo)) setShowSaveToBoard(photo); }} onShareClick={(e: any) => { e.stopPropagation(); setShowShare(photo); }} onRemoveClick={(e: any) => { e.stopPropagation(); deletePin(photo.id); }} />))}</div></div>{displayPhotos.length === 0 && !loading && <div className="empty">{showSaved ? "No saved pins yet." : "Nothing found."}</div>}{!showSaved && <div ref={bottomRef} style={{ padding: "40px", textAlign: "center" }}>{loading && <div className="spinner" />}</div>}</>
        )}

        {showMenu && (<><div className="burger-overlay" onClick={closeAllPanels} /><div className="burger-panel"><div className="burger-header"><span className="burger-logo">GELBET</span><button className="burger-close" onClick={closeAllPanels}>✕</button></div>{user && (<div style={{ padding: "20px 16px", borderBottom: "1px solid #1a1208", display: "flex", alignItems: "center", gap: 14 }}>{userAvatar ? <img src={userAvatar} className="avatar" style={{ width: 40, height: 40 }} alt="" /> : <div className="avatar-placeholder" style={{ width: 40, height: 40, fontSize: 16 }}>{(userName[0] || "U").toUpperCase()}</div>}<div><div style={{ fontWeight: 600, fontSize: 14, color: "#d4b896" }}>{userName}</div><a href="/profile" style={{ color: "#8a6a4a", fontSize: 12, textDecoration: "none" }}>Edit profile</a></div></div>)}<div style={{ padding: "0 16px", fontSize: 10, color: "#4a3520", textTransform: "uppercase", letterSpacing: 2, marginTop: 24, marginBottom: 12 }}>Explore Vibes</div><div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "0 16px 16px" }}>{defaultTags.map(tag => <button key={tag} className="tag-pill" onClick={() => handleTagClick(tag)}>{tag}</button>)}</div><div style={{ height: 1, background: "#1a1208", margin: "10px 0" }} /><div style={{ padding: "10px 0" }}><button className="burger-action" onClick={() => { setShowBoards(true); setShowMenu(false); }}><span className="burger-action-icon" style={{ color: "#c0521a" }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></span><span>My Boards</span></button><button className="burger-action" onClick={() => { setShowSaved(true); setShowMenu(false); }}><span className="burger-action-icon" style={{ color: "#c0521a" }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></span><span>Saved Pins</span></button></div></div></>)}
