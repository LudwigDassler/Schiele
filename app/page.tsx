  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600;700&family=Crimson+Text:ital,wght@0,400;0,600;1,400&family=Syncopate:wght@400;700&display=swap');
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { overflow-x: hidden; background: #050403; font-family: -apple-system, sans-serif; color: white; }
        ...
        .kashmir-scroll::-webkit-scrollbar { width: 4px; } .kashmir-scroll::-webkit-scrollbar-thumb { background: #d4b896; border-radius: 4px; }

        .header { position: sticky; top: 0; z-index: 100; background: rgba(5, 4, 3, 0.85); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.05); padding: 12px 24px; display: flex; align-items: center; justify-content: space-between; }
        .logo { font-family: 'Syncopate', sans-serif; font-size: 20px; font-weight: 700; color: #d4b896; letter-spacing: 0.3em; cursor: pointer; user-select: none; }
        ...
        .search-wrap { display: flex; align-items: center; width: 100%; background: #0a0806; border-radius: 9999px; border: 1px solid rgba(212, 184, 150, 0.2); box-shadow: 0 0 15px rgba(0,0,0,0.5); transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1); } 
        .search-wrap:focus-within { border-color: rgba(212, 184, 150, 0.6); }
        .search-wrap.oracle-active { border-color: #c0521a; box-shadow: 0 0 20px rgba(192, 82, 26, 0.2); }
        .oracle-toggle-inline { display: flex; align-items: center; justify-content: center; padding: 0 12px 0 16px; border-right: 1px solid rgba(212, 184, 150, 0.2); color: rgba(212, 184, 150, 0.6); background: transparent; border-top: none; border-bottom: none; border-left: none; cursor: pointer; transition: color 0.2s; height: 24px; margin: 8px 0; flex-shrink: 0; }
        .oracle-toggle-inline:hover, .search-wrap.oracle-active .oracle-toggle-inline { color: #d4b896; }
        .search-input { width: 100%; padding: 12px 16px; background: transparent; border: none; color: #d4b896; font-size: 14px; outline: none; transition: all 0.3s ease; } 
        .search-input::placeholder { color: rgba(212, 184, 150, 0.4); font-family: -apple-system, sans-serif; }
        .search-wrap.oracle-active .search-input { color: #e3b378; font-family: 'Crimson Text', serif; font-style: italic; font-size: 16px; }
        .search-btn { width: 40px; height: 100%; padding: 0; background: transparent; border: none; color: rgba(212, 184, 150, 0.6); cursor: pointer; transition: color 0.2s; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-right: 8px; } 
        .search-btn:hover { color: #d4b896; }
        
        .search-dropdown { position: absolute; top: calc(100% + 8px); left: 0; width: 100%; background: #0a0806; border: 1px solid rgba(212,184,150,0.2); border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.8); backdrop-filter: blur(12px); z-index: 300; padding: 8px 0; animation: slideUp 0.2s ease; overflow: hidden; }
        .search-dropdown-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px 8px; font-size: 10px; font-weight: 700; color: #8a6a4a; text-transform: uppercase; letter-spacing: 1.5px; }
        ...
        .search-dropdown-item { display: flex; align-items: center; padding: 10px 16px; cursor: pointer; transition: background 0.15s; gap: 12px; color: #d4b896; font-size: 14px; } .search-dropdown-item:hover { background: rgba(212,184,150,0.1); }
        .search-dropdown-icon { font-size: 14px; color: #8a6a4a; } .search-dropdown-text { flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 500; }
        .search-dropdown-remove { background: none; border: none; color: #8a6a4a; cursor: pointer; width: 26px; height: 26px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.2s; } .search-dropdown-remove:hover { color: #e53e3e; background: rgba(229,62,62,0.1); }
        
        .oracle-stage-wrapper { width: 100%; padding: 0 16px; height: 0; opacity: 0; overflow: hidden; transition: all 0.6s cubic-bezier(0.16, 1, 0.3, 1); position: relative; z-index: 10; }
        ...
        .pulse-dot.erratic { animation: pulseStrobe 0.1s infinite alternate; background: #ff3300; box-shadow: 0 0 15px #ff3300; }
        .oracle-track-name { font-family: 'Crimson Text', serif; font-size: 16px; color: #4a3520; font-style: italic; margin-top: 6px; letter-spacing: 1px; transition: all 1s ease; text-align: center; max-width: 400px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .oracle-track-name.active { color: #d4b896; font-weight: 600; }
        
        @keyframes pulseBlink { 0% { opacity: 0.4; filter: drop-shadow(0 0 2px #c0521a); } 100% { opacity: 1; filter: drop-shadow(0 0 10px #c0521a); } }
        @keyframes pulseStrobe { 0% { opacity: 0.2; } 100% { opacity: 1; } }

        @media (max-width: 768px) { .search-form { flex: none; margin: 0; } .search-container { flex: none; } .search-wrap { width: 38px; height: 38px; flex: none; border-radius: 50%; background: transparent; border: none; cursor: pointer; box-shadow: none; } .search-input { display: none; } .search-btn { width: 38px; height: 38px; border-radius: 50%; pointer-events: none; margin: 0; } .oracle-toggle-inline { display: none; } .search-form.mobile-active { position: absolute; left: 16px; right: 16px; top: 10px; z-index: 200; flex: 1; } .search-form.mobile-active .search-container { flex: 1; } .search-form.mobile-active .search-wrap { width: 100%; height: 46px; border-radius: 9999px; background: #0a0806; border: 1px solid rgba(212,184,150,0.2); flex: 1; display: flex; cursor: text; box-shadow: 0 10px 30px rgba(0,0,0,0.9); } .search-form.mobile-active .search-input { display: block; flex: 1; } .search-form.mobile-active .search-btn { width: 40px; pointer-events: auto; } .search-form.mobile-active .oracle-toggle-inline { display: flex; } .search-dropdown { max-width: 100%; } .logo { display: none; } }
        .hbtn { background: transparent; border: none; width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #8a6a4a; flex-shrink: 0; transition: all 0.2s; position: relative; } .hbtn:hover { background: rgba(255,255,255,0.05); color: #d4b896; } .hbtn.active { background: rgba(255,255,255,0.05); color: #c0521a; }
        .avatar { width: 32px; height: 32px; border-radius: 50%; cursor: pointer; border: 2px solid rgba(212,184,150,0.3); flex-shrink: 0; object-fit: cover; } .avatar-placeholder { width: 32px; height: 32px; border-radius: 50%; cursor: pointer; border: 2px solid rgba(212,184,150,0.3); flex-shrink: 0; background: #1a1208; display: flex; align-items: center; justify-content: center; color: #8a6a4a; font-size: 13px; font-weight: 700; }
        .sign-btn { background: transparent; color: #d4b896; border: 1px solid rgba(212,184,150,0.3); border-radius: 20px; padding: 6px 16px; cursor: pointer; font-size: 12px; font-weight: 600; flex-shrink: 0; text-decoration: none; display: flex; align-items: center; transition: all 0.2s; } .sign-btn:hover { border-color: #d4b896; }
        .tag-pill { background: #0a0806; border: 1px solid rgba(212,184,150,0.2); color: #8a6a4a; padding: 6px 16px; border-radius: 20px; font-size: 12px; font-weight: 500; white-space: nowrap; cursor: pointer; transition: all 0.2s; } .tag-pill:hover { background: rgba(212,184,150,0.1); color: #d4b896; } .tag-pill.active { background: rgba(212,184,150,0.1); color: #c0521a; border-color: #c0521a; }
        ...
        .burger-header { padding: 20px 20px 16px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: space-between; } .burger-logo { font-family: 'Syncopate', sans-serif; font-size: 14px; font-weight: 700; letter-spacing: 4px; color: #d4b896; } .burger-close { background: none; border: none; font-size: 18px; cursor: pointer; color: #8a6a4a; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; } .burger-close:hover { color: #d4b896; background: rgba(255,255,255,0.05); } .burger-action { display: flex; align-items: center; gap: 12px; padding: 14px 16px; cursor: pointer; border: none; background: none; width: 100%; text-align: left; color: #8a6a4a; font-size: 13px; transition: all 0.15s; } .burger-action:hover { background: rgba(255,255,255,0.05); color: #d4b896; } .burger-action-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: rgba(0,0,0,0.5); }
        .grid-wrap { padding: 16px; position: relative; z-index: 10; max-width: 1400px; margin: 0 auto; } .masonry { columns: 2; gap: 12px; } @media (min-width: 480px) { .masonry { columns: 3; } } @media (min-width: 768px) { .masonry { columns: 4; } } @media (min-width: 1024px) { .masonry { columns: 5; } } @media (min-width: 1280px) { .masonry { columns: 6; } }
        .boards-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; position: relative; z-index: 10; max-width: 1400px; margin: 0 auto; padding: 0 16px; } @media (min-width: 480px) { .boards-grid { grid-template-columns: repeat(3, 1fr); } } .board-card { border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05); background: rgba(10,8,6,0.8); backdrop-filter: blur(10px); } .board-cover { height: 80px; background: linear-gradient(135deg, #1a1208, #050403); display: flex; align-items: center; justify-content: center; } .board-info { padding: 12px; } .board-actions { display: flex; gap: 8px; margin-top: 10px; } .board-edit-btn, .board-del-btn { flex: 1; padding: 6px; border-radius: 6px; font-size: 11px; cursor: pointer; background: transparent; } .board-edit-btn { border: 1px solid rgba(212,184,150,0.3); color: #8a6a4a; } .board-edit-btn:hover { border-color: #d4b896; color: #d4b896; } .board-del-btn { border: 1px solid #3a1a1a; color: #e53e3e; } .board-del-btn:hover { background: rgba(229,62,62,0.1); }
        ...
        .ghost-btn, .outline-btn, .danger-btn { border-radius: 4px; padding: 12px 24px; cursor: pointer; font-weight: 500; font-size: 13px; transition: all 0.2s; background: transparent; text-transform: uppercase; letter-spacing: 1px; } .ghost-btn { color: #8a6a4a; border: 1px solid transparent; } .ghost-btn:hover { background: rgba(255,255,255,0.05); color: #d4b896; } .outline-btn { color: #8a6a4a; border: 1px solid #2a1f0e; width: 100%; } .outline-btn:hover { border-color: #c0521a; color: #c0521a; } .danger-btn { color: #e53e3e; border: 1px solid #e53e3e; width: 100%; } .danger-btn:hover { background: rgba(229,62,62,0.1); }
        .field { width: 100%; padding: 12px 16px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.1); background: #0a0806; color: #d4b896; font-size: 14px; outline: none; transition: border-color 0.2s; } .field:focus { border-color: #d4b896; }
        ...
        .empty { text-align: center; padding: 100px 20px; color: #8a6a4a; font-size: 16px; font-family: 'Inter', sans-serif; font-style: italic; position: relative; z-index: 10; } .modal-close { background: none; border: none; color: #4a3520; cursor: pointer; font-size: 20px; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; transition: all 0.2s; } .modal-close:hover { background: rgba(255,255,255,0.05); color: #8a6a4a; }
        .toast-container { position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%); background: #0a0806; border: 1px solid rgba(212,184,150,0.3); color: #d4b896; padding: 14px 28px; border-radius: 8px; font-size: 13px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; z-index: 9999; animation: slideUp 0.3s ease; box-shadow: 0 10px 40px rgba(0,0,0,0.9); display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; }
        .ai-global-loader { position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%); z-index: 9999; width: 70px; height: 50px; border-radius: 12px; background: #080604; border: 1px solid #2a1f0e; box-shadow: 0 10px 30px rgba(0,0,0,0.9), 0 0 20px rgba(192,82,26,0.1); animation: slideUp 0.3s ease, floatOrb 3s infinite ease-in-out; display: flex; align-items: center; justify-content: center; padding: 10px; }
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
          <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-[#3a0088] rounded-full blur-[120px] opacity-10 animate-[pulse_10s_ease-in-out_infinite]"></div>
          <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#ff4500] rounded-full blur-[150px] opacity-[0.05]"></div>
      </div>
      ...
      <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <header className="header">
          ...
          <form className={`search-form ${isMobileSearchOpen ? 'mobile-active' : ''}`} onSubmit={handleSearch}>
            <div className="search-container" ref={searchContainerRef}>
              <div className={`search-wrap ${oracleMode ? 'oracle-active' : ''}`} onClick={() => { if (!isMobileSearchOpen) { setIsMobileSearchOpen(true); setTimeout(() => searchInputRef.current?.focus(), 50); } }}>
                ...
                <input 
                  ...
                  placeholder={oracleMode ? "Enter track or artist to synthesize..." : "Search visual archives..."} 
                  value={search} 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {user ? (<a href="/profile" style={{ marginLeft: 8 }}>{userAvatar ? <img src={userAvatar} className="avatar" alt="avatar" /> : <div className="avatar-placeholder">{(userName[0] || "U").toUpperCase()}</div>}</a>) : (<a href="/auth" className="sign-btn" style={{ marginLeft: 8 }}>Enter</a>)}
            
            {/* KASHMIR CORE BLOB AVATAR (NO BADGE) */}
            <div className="relative ml-2" ref={kashmirRef}>
                <div className="w-10 h-10 rounded-full border border-[#d4b896]/30 p-0.5 relative hover:border-[#d4b896] transition-colors shadow-[0_0_15px_rgba(0,0,0,0.8)] cursor-pointer group" onClick={(e) => { e.stopPropagation(); setIsKashmirOpen(!isKashmirOpen); }}>
                    <div className="w-full h-full rounded-full overflow-hidden relative bg-[#0a0806] flex items-center justify-center">
                        <div className="absolute inset-0 opacity-80 mix-blend-screen scale-125 filter blur-[3px]">
                            <div className="absolute w-[80%] h-[80%] -top-[10%] left-0 rounded-full" style={{ backgroundColor: activePersona.colors[0], animation: 'ooze 10s infinite alternate ease-in-out' }}></div>
                            <div className="absolute w-[70%] h-[70%] -bottom-[10%] right-0 rounded-full" style={{ backgroundColor: activePersona.colors[1], animation: 'ooze 8s infinite alternate-reverse ease-in-out' }}></div>
                            <div className="absolute w-[60%] h-[60%] top-[20%] left-[20%] rounded-full opacity-80" style={{ backgroundColor: activePersona.colors[2], animation: 'ooze 12s infinite alternate ease-in-out' }}></div>
                        </div>
                        <div className="absolute inset-0 opacity-40 mix-blend-screen force-fluid-filter" style={{ background: 'repeating-linear-gradient(-45deg, transparent, transparent 1px, rgba(255,255,255,0.2) 2px, rgba(255,255,255,0.2) 3px)', animation: 'flow-lines 10s linear infinite' }}></div>
                        <div className="absolute inset-0 shadow-[inset_0_0_10px_rgba(0,0,0,0.9)] rounded-full"></div>
                    </div>
                </div>

                {/* Dropdown Menu */}
                {isKashmirOpen && (
                    <div className="absolute right-0 mt-4 w-64 bg-[#0a0806] border border-[#d4b896]/20 rounded-2xl p-2 shadow-[0_20px_50px_rgba(0,0,0,0.9)] z-50">
                        <div className="text-[10px] font-syncopate tracking-[0.2em] text-[#d4b896] p-4 text-center border-b border-[#d4b896]/10 mb-2">KASHMIR CORE</div>
                        <div className="max-h-[300px] overflow-y-auto kashmir-scroll py-2">
                            {personas.map(p => (
                                <div key={p.id} className="flex items-center gap-4 p-3 hover:bg-[#d4b896]/10 rounded-xl transition-colors cursor-pointer group" onClick={() => { setActivePersona(p); setIsKashmirOpen(false); }}>
                                    <div className="w-2 h-2 rounded-full group-hover:shadow-[0_0_10px_currentColor] transition-shadow shrink-0" style={{ backgroundColor: p.hex }}></div>
                                    <div>
                                        <div className="text-sm text-[#d4b896] font-medium leading-tight">{p.name}</div>
                                        <div className="text-[10px] text-[#d4b896]/50 italic mt-0.5">{p.sub}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
        ...
        {(showSaved || showBoards) && (<div style={{ padding: "12px 20px", fontSize: 13, color: "#8a6a4a", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 12, position: "relative", zIndex: 10 }}>{showSaved && <span>Saved Collection (<span style={{ color: "#d4b896" }}>{pins.length}</span>)</span>}{showBoards && <span>My Boards (<span style={{ color: "#d4b896" }}>{boards.length}</span>)</span>}</div>)}

        {showBoards && (
          <div className="boards-grid" style={{ paddingTop: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, gridColumn: "1 / -1" }}><h2 style={{ fontSize: 16, fontWeight: 700, color: "#d4b896", fontFamily: "Syncopate, sans-serif", letterSpacing: 2 }}>MY BOARDS</h2><button className="primary-btn" style={{ width: "auto", padding: "8px 16px", fontSize: 11 }} onClick={() => setShowNewBoard(true)}>+ New Board</button></div>
            {boards.length === 0 ? <div className="empty" style={{ gridColumn: "1 / -1" }}>No boards yet. Create your first collection.</div> : (boards.map(board => (<div key={board.id} className="board-card"><div className="board-cover"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2a1f0e" strokeWidth="1.5"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></div><div className="board-info"><div style={{ fontSize: 14, fontWeight: 600, color: "#d4b896" }}>{board.name}</div>{board.description && <div style={{ fontSize: 12, color: "#8a6a4a", marginTop: 4, fontStyle: "italic" }}>{board.description}</div>}<div style={{ fontSize: 11, color: "#8a6a4a", marginTop: 8 }}>{pins.filter(p => p.board_id === board.id).length} pins</div><div className="board-actions"><button className="board-edit-btn" onClick={() => setEditBoard(board)}>Edit</button><button className="board-del-btn" onClick={() => deleteBoard(board.id)}>Delete</button></div></div></div>)))}
          </div>
        )}

        {!showBoards && (
          <><div className="grid-wrap"><div className="masonry">{displayPhotos.map((photo, i) => (<PinCard key={`${photo.id}-${i}`} photo={photo} nsfwAllowed={nsfwAllowed} isPinned={isPinned(photo)} showSaved={showSaved} onClick={() => { feedLocalAI(photo.src, photo.id); const isBlurred = photo.isNsfw && !nsfwAllowed; if (isBlurred) setShowAgeGate(true); else router.push(`/vibe?src=${encodeURIComponent(photo.src)}&title=${encodeURIComponent(photo.title || "")}&link=${encodeURIComponent(photo.link || "")}`); }} onSaveClick={(e: any) => { e.stopPropagation(); if (!isPinned(photo)) savePin(photo); }} onShareClick={(e: any) => { e.stopPropagation(); sharePhoto(photo); }} onRemoveClick={(e: any) => { e.stopPropagation(); deletePin(photo.id); }} />))}</div></div>{displayPhotos.length === 0 && !loading && <div className="empty">{showSaved ? "Archive is empty." : "Nothing found."}</div>}{!showSaved && <div ref={bottomRef} style={{ padding: "40px", textAlign: "center", position: "relative", zIndex: 10 }}>{loading && <div className="spinner" />}</div>}</>
        )}

        {showMenu && (<><div className="burger-overlay" onClick={closeAllPanels} /><div className="burger-panel"><div className="burger-header"><span className="burger-logo">GELBET</span><button className="burger-close" onClick={closeAllPanels}>✕</button></div>{user && (<div style={{ padding: "24px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 14 }}>{userAvatar ? <img src={userAvatar} className="avatar" style={{ width: 44, height: 44 }} alt="" /> : <div className="avatar-placeholder" style={{ width: 44, height: 44, fontSize: 16 }}>{(userName[0] || "U").toUpperCase()}</div>}<div><div style={{ fontWeight: 600, fontSize: 14, color: "#d4b896" }}>{userName}</div><a href="/profile" style={{ color: "#8a6a4a", fontSize: 12, textDecoration: "none", marginTop: 4, display: "block" }}>Edit profile</a></div></div>)}<div style={{ padding: "0 16px", fontSize: 10, color: "#8a6a4a", textTransform: "uppercase", letterSpacing: 2, marginTop: 24, marginBottom: 16, fontWeight: "bold" }}>Explore Archives</div><div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "0 16px 20px" }}>{defaultTags.map(tag => <button key={tag} className="tag-pill" onClick={() => handleTagClick(tag)}>{tag}</button>)}</div><div style={{ height: 1, background: "rgba(255,255,255,0.05)", margin: "10px 0" }} /><div style={{ padding: "10px 0" }}><button className="burger-action" onClick={() => { setShowBoards(true); setShowMenu(false); }}><span className="burger-action-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg></span><span>My Boards</span></button><button className="burger-action" onClick={() => { setShowSaved(true); setShowMenu(false); }}><span className="burger-action-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></span><span>Saved Pins</span></button></div></div></>)}

        {showNewBoard && (
          <div className="modal-backdrop" style={{ zIndex: 300 }} onClick={() => setShowNewBoard(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#0a0806", border: "1px solid rgba(212,184,150,0.2)", borderRadius: 16, padding: 32, maxWidth: 400, width: "100%", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 20px 50px rgba(0,0,0,0.9)" }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: "#d4b896", fontFamily: "Syncopate, sans-serif", letterSpacing: 2 }}>CREATE NEW ARCHIVE</h2>
        ...
        {showAIModal && (<div className="modal-backdrop" onClick={() => setShowAIModal(false)}><div onClick={e => e.stopPropagation()} style={{ background: "#0a0806", border: "1px solid rgba(212,184,150,0.2)", borderRadius: 16, padding: 32, maxWidth: 440, width: "100%", display: "flex", flexDirection: "column", gap: 16, animation: "slideUp 0.3s ease", boxShadow: "0 20px 50px rgba(0,0,0,0.9)" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><h2 style={{ fontSize: 14, fontWeight: 700, color: "#8a6a4a", fontFamily: "Syncopate, sans-serif", letterSpacing: 2 }}>AI VIBE ASSISTANT</h2><button className="modal-close" onClick={() => setShowAIModal(false)}>✕</button></div><textarea className="field" placeholder="Describe a specific mood or aesthetic..." value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} style={{ height: 100, resize: "none" }} /><div style={{ display: "flex", gap: 12, marginTop: 10 }}><button className="ghost-btn" onClick={() => setShowAIModal(false)}>Cancel</button><button className="primary-btn" style={{ flex: 1, opacity: !aiPrompt.trim() ? 0.4 : 1 }} onClick={handleAIGenerate}>Synthesize</button></div></div></div>)}

        {showAgeGate && (
          <div className="modal-backdrop" style={{ zIndex: 99999 }} onClick={() => setShowAgeGate(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#0a0806", border: "1px solid rgba(212,184,150,0.2)", borderRadius: 16, padding: "40px", maxWidth: 440, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 24, animation: "slideUp 0.3s ease", boxShadow: "0 20px 60px rgba(0,0,0,0.9)" }}>
              <div style={{ textAlign: "center" }}>
                <h2 style={{ fontSize: 18, fontWeight: 600, color: "#d4b896", fontFamily: "Syncopate, sans-serif", letterSpacing: 4, margin: "0 0 12px 0" }}>RESTRICTED ARCHIVE</h2>
                <p style={{ color: "#8a6a4a", fontSize: 14, lineHeight: 1.6, fontStyle: "italic", margin: 0 }}>This sector contains sensitive visual material.<br/>Confirmation of maturity is required to proceed.</p>
              </div>
        ...
        {showSaveToBoard && (<div className="modal-backdrop" onClick={() => setShowSaveToBoard(null)}><div onClick={e => e.stopPropagation()} style={{ background: "#0a0806", border: "1px solid rgba(212,184,150,0.2)", borderRadius: 16, padding: 32, maxWidth: 400, width: "100%", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 20px 50px rgba(0,0,0,0.9)" }}><button className="primary-btn" onClick={() => savePin(showSaveToBoard)}>Save directly</button><button className="ghost-btn" onClick={() => { setShowNewBoard(true); setShowSaveToBoard(null); }}>+ Create new board</button></div></div>)}
        
        {isAILoading && <div className="ai-global-loader"><div className="spinner" style={{ width: 20, height: 20, borderWidth: 2 }} /></div>}
